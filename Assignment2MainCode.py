...
class AdminSystem:
    ...
    def partition_students():
        ...
        print("        \033[33m PASS/FAIL partitiion \033[0m")

        # Need fail/pass array to be printed, code below prints one output and if 1 grade = F entire entry printed as fail, need someway to print array 
        # FAIL [student 1, student 2], PASS [student 1, student2] 

        for student in fail_students:
               print(f"         FAIL --> [{student.name} :: {student.id} --> GRADE: F - MARK: {subject.mark}]") 

        for student in pass_students:
            if not any(subject.calculate_grade() == 'F' for subject in student.subjects):
                if (subject.mark >= 85):
                    print(f"         PASS --> [{student.name} :: {student.id} --> GRADE: HD - MARK: {subject.mark}")

                elif(subject.mark >= 75):
                    print(f"          PASS --> [{student.name} :: {student.id} --> GRADE: HD - MARK: {subject.mark}]")

                elif(subject.mark >= 65):
                    print(f"          PASS --> [{student.name} :: {student.id} --> GRADE: HD - MARK: {subject.mark}]")

                elif(subject.mark >= 45):
                    print(f"          PASS --> [{student.name} :: {student.id} --> GRADE: HD - MARK: {subject.mark}]")
    ...
    def remove_student():
        ...
        try:
            ...
            # Exception error is written in the data file
            with open("students.data","w") as file:
                for student in students:
                    file.write(f"{student.id},{student.name},{student.email},{student.password}")               
                    for subject in student.subjects:
                        file.write(f",{subject,id}:{subject.mark}")
                    file.write("/n") 
                    print(f"        \033[33m Removing student {student_id} account \033[0m")
                # if student doesnt exist
                #if (student_id):
                #    print(f"        \033[33m Student {student_id} does not exist \033[0m")
    ...
    def group_student_by_grade():
        ...
        if group_students:
            # Not sure if the average of grades need to be caculated before showing final grade
            print(f"         \033[33m Grade grouping \033[0m")   
            for student,student_subjects in group_students:
                if (subject.mark >= 85):
                    print(f"          HD --> [{student.name} :: {student.id} --> GRADE: HD - MARK: {subject.mark}]")

                elif(subject.mark >= 75):
                    print(f"          D --> [{student.name} :: {student.id} --> GRADE: D - MARK: {subject.mark}]")

                elif(subject.mark >= 65):
                    print(f"          C --> [{student.name} :: {student.id} --> GRADE: C - MARK: {subject.mark}]")

                elif(subject.mark >= 45):
                    print(f"          P --> [{student.name} :: {student.id} --> GRADE: P - MARK: {subject.mark}]")

                elif(subject.mark <= 44):
                    print(f"          F --> [{student.name} :: {student.id} --> GRADE: F - MARK: {subject.mark}]")

        else:
            print(f"         \033[31m No student found with grade \033[0m")  
